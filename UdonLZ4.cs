...
namespace jp.ootr.UdonLZ4
{
    ...
    public class UdonLZ4 : UdonSharpBehaviour
    {
        ...
        private long _lz4SIndex = 0;
        private long _lz4DIndex = 0;
        private long _lz4SLength = 0;
        private long _lz4SEnd = 0;
        private bool _lz4HasBlockSum = false;
        private float _lz4StartTime = 0;
        
        public byte[] Decompress(byte[] src, long maxSize = -1)
        {
            if (!ValidateData(src, out var contentIndex, out var hasBlockSum, out var hasContentSum, out var hasContentSize, out var hasDictId, out var maxBlockSize, out var error))
            {
        ...
        public void __DecompressItemAsync()
        {
            ...
            if (!ValidateData(_lz4Buffer[0], out var contentIndex, out var hasBlockSum, out var hasContentSum, out var hasContentSize, out var hasDictId, out var maxBlockSize, out var error))
            {
        ...
        public void __DecompressBlockInternalAsync()
        {
            ...
            {
                if (DecompressBlockInternal(_lz4Buffer[0], _lz4Dist, ref _lz4SIndex, ref _lz4DIndex, _lz4SEnd))
                {
                    break;
                }
        ...
        private bool ValidateData(byte[] src, out long contentIndex, out bool hasBlockSum, out bool hasContentSum, out bool hasContentSize, out bool hasDictId, out long maxBlockSize, out DecompressError error)
        {
            contentIndex = -1;
            hasBlockSum = false;
            hasContentSum = false;
        ...
        private long DecompressFrame(byte[] src, byte[] dist, long sIndex, bool useBlockSum )
        {
            ...
            while (true)
            {
                uint compSize = ReadU32(src, ref sIndex);
                if (compSize == 0)
                {
                    break;
                }
        ...
        private bool DecompressBlockInternal(byte[] src, byte[] dst, ref long sIndex, ref long dIndex, long sEnd)
        {
            ...
            if (mOffset == 1)
            {
                for (long j = dIndex; j < dIndex+mLength; j++)
                {
                    dst[j] = (byte)(dst[dIndex - 1] | 0);   
                }
                dIndex += mLength;
            }
            else
